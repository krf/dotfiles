...
sudo apt-get install -y ack-grep acpi automake bzr build-essential clang-3.5 cgdb deborphan gimp git git-svn imagemagick inxi kdesdk-scripts kdiff3 kolourpaint linux-tools-common mercurial mesa-utils nethogs ninja-build pastebinit powertop valgrind vim wajig zsh `# CLI` \
