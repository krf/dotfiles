...
sudo apt-get install -y ack-grep acpi apt-file automake bzr build-essential ccache clang-3.5 cgdb colordiff deborphan filelight gimp git git-svn gscan2pdf htop imagemagick inxi kdesdk-scripts kdiff3 kolourpaint4 libnofify-bin linux-tools-common mercurial mesa-utils nethogs ninja-build nmap pandoc pastebinit powertop python-rbtools qt5-doc sloccount texlive-latex-base texlive-fonts-recommended valgrind vim wajig wdiff whois zsh `# CLI` \
