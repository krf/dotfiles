...
sudo apt-get install -y ack-grep acpi automake bzr build-essential clang-3.5 cgdb deborphan filelight gimp git git-svn imagemagick inxi kdesdk-scripts kdiff3 kolourpaint linux-tools-common mercurial mesa-utils nethogs ninja-build pandoc pastebinit powertop texlive-latex-base texlive-fonts-recommended valgrind vim wajig zsh `# CLI` \
